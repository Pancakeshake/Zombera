...
public class Script
{
    public static string Main()
    {
        ...
        // 1) Scan all threads 0..30 across sampled frames for any node with "Road"
        ...
        // 2) Dump top 2 levels of hierarchy for the LAST frame, main thread
            ...
            foreach (var k in kids)
            {
                ...
                if (grand != null)
                    foreach (var g in grand)
                    {
                        ...
                        float gms = 0f;
                        try { gms = (float)getCol.Invoke(lastView, new object[] { (int)g, 0 }); } catch { }
                        sb.Append("[PCR]     - ").Append(gn).Append("  ").Append(gms.ToString("F2")).Append("ms").AppendLine();
                    }
            }
        return sb.ToString();
    }

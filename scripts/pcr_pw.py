...
public class Script
{
    ...
    {
        var t = typeof(UnityEditor.ProfilerWindow);
        var sb = new StringBuilder();
        foreach (var m in t.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static))
        {
            if (m.Name.ToLower().Contains("record") || m.Name.ToLower().Contains("capture") || m.Name.ToLower().Contains("connect"))
            {
                var ps = new StringBuilder();
                foreach (var p in m.GetParameters()) ps.Append(p.ParameterType.Name).Append(",");
                sb.Append("[PCR-M] ").Append(m.IsStatic ? "static " : "").Append(m.Name).Append("(").Append(ps).Append(") -> ").Append(m.ReturnType.Name).AppendLine();
            }
        }

...
{
   public class UnityURP6p3RenderLoopAdapter : IRenderLoopAdapter
   {
      ...
      {
         return "_MSRENDERLOOP_UNITYURP6P3";
      }
      ...
      public void Init(string[] paths)
      {
         ...
         {
            string p = paths[i];
            if (p.EndsWith("microsplat_template_urp6p3_template.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_urp6p3_passterrain.txt"))
            {
            ...
            else if (p.EndsWith("microsplat_template_urp6p3_passdepthnormals.txt"))
            {
               templatePassDepthNormals = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_passforward.txt"))
            {
               templatePassForward = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_passdepthonly.txt"))
            {
               templatePassDepthOnly = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_passmeta.txt"))
            {
               templatePassMeta = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_passhadows.txt"))
            {
               templatePassShadow = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith ("microsplat_template_urp6p3_passgbuffer.txt"))
            {
               templatePassGBuffer = AssetDatabase.LoadAssetAtPath<TextAsset> (p);
            }
            else if (p.EndsWith ("microsplat_template_urp6p3_passmotion.txt"))
            {
               templatePassMotion = AssetDatabase.LoadAssetAtPath<TextAsset> (p);
            }
            else if (p.EndsWith("microsplat_template_shared.txt"))
            {
               templateShared = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_vert.txt"))
            {
               templateVert = AssetDatabase.LoadAssetAtPath<TextAsset>(p);
            }
            else if (p.EndsWith("microsplat_template_urp6p3_include.txt"))
            {
         ...
         if (templateURP == null || templateInclude == null || templateShared == null || templateVert == null
            || templatePassForward == null || templateChain == null || templatePassGBuffer == null || templatePassDepthNormals == null || templatePassMotion == null)
         {
            Debug.LogError ("URP Template files not found, will not be able to compile shaders");
         }
      }

      StringBuilder BuildTemplate(Blocks blocks, string[] features, string fallbackOverride = null)
      {
         ...
         var passMotion = new StringBuilder(templatePassMotion.text);
         ...
         {
            template = template.Replace("%PASSMOTIONVECTOR%", "");
         }
         else
         {
         ...
         header.AppendLine("//   Render Pipeline: URP6.3");
            ...
            string baseName)
      {
         ...
         var shader = BuildTemplate(blocks, features, baseName);
         ...
         // need to alias these two so the replace wont nuke them
         shader = shader.Replace("UNITY_MATRIX_PREV_VP", "temp1");
         shader = shader.Replace("UNITY_MATRIX_PREV_I_VP", "temp2");
         shader = shader.Replace("UNITY_MATRIX_P", "GetViewToHClipMatrix()");
         shader = shader.Replace("temp1", "UNITY_MATRIX_PREV_VP");
         shader = shader.Replace("temp2", "UNITY_MATRIX_PREV_I_VP");

...
namespace JBooth.MicroSplat
{
   ...
   public class MicroSplatProceduralTexture : FeatureDescriptor
   {
      ...
      {
         return "https://docs.google.com/document/d/1UeWX6RwblgFHhyZqvT0tDA6obo4PaPm-0lBW8BLkmPs/edit?usp=sharing";
      }
      ...
      public AlphaMode alphaMode = AlphaMode.ReplaceTextureIndex;
      ...
      GUIContent CAlphaMode = new GUIContent("Alpha Mode", "When combining with splat maps, you can decide that one index means procedural goes where ever this is painted, or you can decide to pick up to 4 textures that will always stay untouched by procedural texturing");
      ...
      public override void DrawFeatureGUI (MicroSplatKeywords keywords)
      {
         ...
         if (proceduralTexture)
         {
            ...
            if (combineWithSplat == true)
            {
               EditorGUI.indentLevel++;
               alphaMode = (AlphaMode)EditorGUILayout.EnumPopup(CAlphaMode, alphaMode);
               EditorGUI.indentLevel--;
            }
      ...
      public static MicroSplatProceduralTextureConfig FindOrCreateProceduralConfig (Material mat)
      {
         ...
         if (cfg == null)
         {
            ...
            var msos = GameObject.FindObjectsByType<MicroSplatObject>(FindObjectsSortMode.None);
#else
      ...
      {
         int count = EditorGUILayout.DelayedIntField ("Layer Count", cfg.layers.Count);
      ...
      int DrawPreserveIndex(int index, Texture2DArray ta)
      {
         bool state = index >= 0;
         EditorGUILayout.BeginHorizontal();
         state = EditorGUILayout.Toggle(state, GUILayout.Width(80));
         if (index < 0)
            index = 0;
         var old = GUI.enabled;
         GUI.enabled = state;
         index = MicroSplatUtilities.DrawTextureSelector(index, ta, true);
         GUI.enabled = old;
         EditorGUILayout.EndHorizontal();
         if (!state)
            index = -1;
      ...
      public override void DrawShaderGUI (MicroSplatShaderGUI shaderGUI, MicroSplatKeywords keywords, Material mat, MaterialEditor materialEditor, MaterialProperty [] props)
      {
         ...
         if ((proceduralTexture || heightGradients || heightHSV) && MicroSplatUtilities.DrawRollup ("Procedural Texture"))
         {
            ...
            else if (proceduralTexture && combineWithSplat && alphaMode == AlphaMode.PreserveTextureIndexes && mat.HasProperty("_PCPreserveIndexes"))
            {
               var v4 = mat.GetVector("_PCPreserveIndexes");
               var old = v4;
               var ta = (Texture2DArray)mat.GetTexture("_Diffuse");
               EditorGUILayout.LabelField("Textures to Preserve");
               EditorGUILayout.BeginHorizontal();
               v4.x = DrawPreserveIndex(Mathf.RoundToInt(v4.x), ta);
               v4.y = DrawPreserveIndex(Mathf.RoundToInt(v4.y), ta);
               EditorGUILayout.EndHorizontal();
               EditorGUILayout.BeginHorizontal();
               v4.z = DrawPreserveIndex(Mathf.RoundToInt(v4.z), ta);
               v4.w = DrawPreserveIndex(Mathf.RoundToInt(v4.w), ta);
               EditorGUILayout.EndHorizontal();
               if (old != v4)
               {
                  mat.SetVector("_PCPreserveIndexes", v4);
                  EditorUtility.SetDirty(mat);
               }
            }
      ...
      public override void WriteProperties (string [] features, System.Text.StringBuilder sb)
      {
         ...
         if (proceduralTexture)
         {
            if (combineWithSplat)
            {
               if (alphaMode == AlphaMode.PreserveTextureIndexes)
               {
                  sb.AppendLine("      _PCPreserveIndexes(\"Indexes to preserve\", Vector) = (-1,-1,-1,-1)");
               }
               else
               {
      ...
      public override string [] Pack ()
      {
         ...
         if (proceduralTexture)
         {
            ...
            if (combineWithSplat)
            {
               ...
               if (alphaMode == AlphaMode.PreserveTextureIndexes)
               {
                  features.Add(GetFeatureName(DefineFeature._PCPRESERVEINDEXES));
               }
            }
      ...
      public override void Unpack (string [] keywords)
      {
         ...
         if (proceduralTexture)
         {
            ...
            alphaMode = AlphaMode.ReplaceTextureIndex;
            if (HasFeature(keywords, DefineFeature._PCPRESERVEINDEXES))
               alphaMode = AlphaMode.PreserveTextureIndexes;

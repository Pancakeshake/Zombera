...
# SOURCE LAYOUT (AI-FRIENDLY)

Gameplay code lives under `Assets/Scripts/` so searches don’t mix scripts with art assets.

- `Assets/Scripts/Core/`: high-level game flow + shared state (e.g. `GameManager`, session/startup coordination)
- `Assets/Scripts/Systems/`: singleton-style runtime systems/managers (save/time/events/unit registries, etc.)
- `Assets/Scripts/World/`, `Assets/Scripts/Characters/`, `Assets/Scripts/AI/`, `Assets/Scripts/Combat/`, etc.: game logic grouped by domain
- `Assets/Editor/`: editor-only tooling (kept separate so it never compiles into runtime builds)
- `Archive/_Recovery*`: quarantined historical/backup assets outside `Assets/` so Unity + AI global searches don’t see duplicate scripts
...
- Prefer `*Manager` for scene/global orchestration singletons (registries, coordination, tick scheduling).
- Prefer `*Controller` for per-entity gameplay control components (player/zombie/squad movement/combat controllers).
- Prefer `*AI` only when it is a specific AI strategy/brain implementation (and not a general “controller”).

Compatibility note: when renaming a Unity component, keep a thin legacy wrapper (old class name)
that inherits the new implementation so existing scenes/prefabs do not break.

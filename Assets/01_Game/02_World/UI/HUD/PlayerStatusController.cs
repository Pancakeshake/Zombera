...
namespace Zombera.UI
{
    ...
    public sealed class PlayerStatusController : MonoBehaviour
    {
        ...
        [Header("Portrait")] [SerializeField] private Image portraitImage;

        [Header("Text")] [SerializeField] private TextMeshProUGUI playerNameText;

        [SerializeField] private TextMeshProUGUI conditionText;
        [SerializeField] private TextMeshProUGUI healthValueText;
        [SerializeField] private TextMeshProUGUI staminaValueText;

        [Header("Bars")] [SerializeField] private Slider healthSlider;

        [SerializeField] private Slider staminaSlider;

        private UnitHealth _boundHealth;
        private UnitStats _boundStats;
        ...
        public void BindUnit(Unit unit)
        {
            ...
            _boundHealth = unit.Health;
            _boundStats = unit.Stats;

            if (_boundHealth != null)
            {
                _boundHealth.Damaged += OnHealthChanged;
                _boundHealth.Healed += OnHealthChanged;
            }

            if (_boundStats != null) _boundStats.StaminaChanged += OnStaminaChanged;
        ...
        public void UnbindUnit()
        {
            ...
            _boundStats.StaminaChanged -= OnStaminaChanged;
            _boundStats = null;
        }
        ...
        {
            var health01 = 1f;
            if (_boundHealth != null && _boundHealth.MaxHealth > 0f)
                health01 = Mathf.Clamp01(_boundHealth.CurrentHealth / _boundHealth.MaxHealth);
            SetNormalizedValue(healthSlider, healthValueText, health01);

            var stamina01 = 1f;
            if (_boundStats != null && _boundStats.MaxStamina > 0f)
                stamina01 = Mathf.Clamp01(_boundStats.Stamina / _boundStats.MaxStamina);
        ...
        public void SetStatus(PlayerStatusViewData statusData)
        {
            if (playerNameText != null)
                playerNameText.text =
                    string.IsNullOrWhiteSpace(statusData.playerName) ? "Player" : statusData.playerName;

            if (conditionText != null)
                conditionText.text = string.IsNullOrWhiteSpace(statusData.conditionText)
                    ? "Stable"
                    : statusData.conditionText;
        ...
        private static void SetNormalizedValue(Slider slider, TextMeshProUGUI valueLabel, float value01)
        {
            ...
            if (valueLabel != null) valueLabel.text = $"{Mathf.RoundToInt(clamped * 100f)}%";
        }
    }

...
//   Render Pipeline: URP6.3
//   Platform: WindowsEditor
////////////////////////////////////////


Shader "MicroSplat/ZomberaTerrain_TerrainObjectBlend"
{
   Properties
   {
      ...
      _AlphaHoleTexture("ClipMap", 2D) = "white" {}
     ...
     _DistanceResampleNormalStrength("Resampled Normal Strength", Range(0.1, 1.3)) = 1
      ...
      _GlobalNormalFade("Global Normmal Fade", Vector) = (50, 150, 1, 1)
      _GlobalNormalUVScale("UV Scale", Vector) = (1, 1, 0, 0)
   ...
   SubShader
   {
            Tags {"RenderPipeline" = "UniversalPipeline"  "RenderType" = "UniversalLitShader" "Queue" = "Transparent+100" "IgnoreProjector" = "False"  "TerrainCompatible" = "true" "SplatCount" = "20"}
...
ZWrite Off
   ...
   #pragma fragment Frag
      ...
      #define _MAX20TEXTURES 1
      #define _CONTROLNOISEUV 1
      #define _BRANCHSAMPLES 1
      #define _BRANCHSAMPLESAGR 1
      #define _ALPHAHOLETEXTURE 1
      #define _DETAILNOISE 1
      #define _DISTANCENOISE 1
      #define _PERTEXDISTANCENOISESTRENGTH 1
      #define _NOISEHEIGHT 1
      #define _NOISEHEIGHTFBM 1
      #define _DISTANCERESAMPLE 1
      #define _DISTANCERESAMPLENORMAL 1
      #define _PERTEXDISTANCERESAMPLESTRENGTH 1
      #define _PERTEXDISTANCERESAMPLEUVSCALE 1
      #define _DISTANCERESAMPLEMAXLAYER3 1
      #define _NORMALNOISE 1
      #define _ANTITILEARRAYNORMAL 1
      #define _ANTITILEARRAYDETAIL 1
      #define _ANTITILEARRAYDISTANCE 1
      #define _ANTITILEPERTEX 1
      #define _ANTITILETRIPLANAR 1
...
#if _GLOBALTINT2 || _GLOBALTINT3 || _GLOBALTINT4 || _GLOBALTINT5
      float4 _GlobalTintBlends;
      float4 _GlobalTintFade2;
      float4 _GlobalTintTex2_TexelSize;
      float4 _GlobalTintUVScale2;
#endif
#if _GLOBALTINT3 || _GLOBALTINT4 || _GLOBALTINT5
      float4 _GlobalTintFade3;
      float4 _GlobalTintTex3_TexelSize;
      float4 _GlobalTintUVScale3;
#endif
#if _GLOBALTINT4 || _GLOBALTINT5
      float4 _GlobalTintFade4;
      float4 _GlobalTintTex4_TexelSize;
      float4 _GlobalTintUVScale4;
#endif
#if _GLOBALTINT5
      float4 _GlobalTintFade5;
      float4 _GlobalTintTex5_TexelSize;
      float4 _GlobalTintUVScale5;
#endif
...
void PrepareStochasticUVs(float scale, float2 uv, out float2 uv1, out float2 uv2, out float2 uv3, out half3 weights)
{
   ...
   weights = half3(w1, w2, w3);
...
#if _GLOBALTINT2 || _GLOBALTINT3 || _GLOBALTINT4 || _GLOBALTINT5
      TEXTURE2D(_GlobalTintTex2);
#endif
#if _GLOBALTINT3 || _GLOBALTINT4 || _GLOBALTINT5
      TEXTURE2D(_GlobalTintTex3);
#endif
#if _GLOBALTINT4 || _GLOBALTINT5
      TEXTURE2D(_GlobalTintTex4);
#endif
#if _GLOBALTINT5
      TEXTURE2D(_GlobalTintTex5);
#endif
      ...
      half4 SampleGlobalTintTex(float2 uv, TEXTURE2D(tintTex), float4 texelSize)
      {
         half4 ret = 0;
            ...
            float2 w1 = uv + c;
            #if _GLOBALTEXWRAP
               half4 s = SAMPLE_TEXTURE2D(tintTex, sampler_Diffuse, float2(w0.x, w0.y))
                 + SAMPLE_TEXTURE2D(tintTex, sampler_Diffuse, float2(w0.x, w1.y))
                 + SAMPLE_TEXTURE2D(tintTex, sampler_Diffuse, float2(w1.x, w1.y))
                 + SAMPLE_TEXTURE2D(tintTex, sampler_Diffuse, float2(w1.x, w0.y));
            #else
               half4 s = SAMPLE_TEXTURE2D(tintTex, shared_linear_clamp_sampler, float2(w0.x, w0.y))
                 + SAMPLE_TEXTURE2D(tintTex, shared_linear_clamp_sampler, float2(w0.x, w1.y))
                 + SAMPLE_TEXTURE2D(tintTex, shared_linear_clamp_sampler, float2(w1.x, w1.y))
                 + SAMPLE_TEXTURE2D(tintTex, shared_linear_clamp_sampler, float2(w1.x, w0.y));
            #endif
            ret = s / 4.0;
         #else
            #if _GLOBALTEXWRAP
               ret = SAMPLE_TEXTURE2D(tintTex, sampler_Diffuse, uv);
            #else
               ret = SAMPLE_TEXTURE2D(tintTex, shared_linear_clamp_sampler, uv);
            #endif
         #endif
         return ret;
      }
      ...
      #if _GLOBALTINT2
      #define BLEND_GLOBAL_TINT_PERTEX(rawSamples, config, camDist, slopeFilter, noiseUV) \
         GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex,  _GlobalTintTex_TexelSize, _GlobalTintUVScale, _GlobalTintFade, _GlobalTextureParams.x, 5.5, 1); \
         GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x, 30.5, 0); \

      #elif _GLOBALTINT3
         #define BLEND_GLOBAL_TINT_PERTEX(rawSamples, config, camDist, slopeFilter, noiseUV) \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex,  _GlobalTintTex_TexelSize, _GlobalTintUVScale, _GlobalTintFade, _GlobalTextureParams.x, 5.5, 1); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x, 30.5, 0); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y, 30.5, 1); \
         ...
         #define BLEND_GLOBAL_TINT_PERTEX(rawSamples, config, camDist, slopeFilter, noiseUV) \
            ...
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x, 30.5, 0); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y, 30.5, 1); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex4, _GlobalTintTex4_TexelSize, _GlobalTintUVScale4, _GlobalTintFade4, _GlobalTintBlends.z, 30.5, 2); \
         ...
         #define BLEND_GLOBAL_TINT_PERTEX(rawSamples, config, camDist, slopeFilter, noiseUV) \
            ...
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y, 30.5, 1); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex4, _GlobalTintTex4_TexelSize, _GlobalTintUVScale4, _GlobalTintFade4, _GlobalTintBlends.z, 30.5, 2); \
            GlobalTintTexturePerTex(rawSamples, config, camDist, slopeFilter, noiseUV, _GlobalTintTex5, _GlobalTintTex5_TexelSize, _GlobalTintUVScale5, _GlobalTintFade5, _GlobalTintBlends.w, 30.5, 3); \
      ...
      void GlobalTintTexturePerTex(inout RawSamples s, Config config, float camDist, float slopeFilter, float2 noiseUV, 
         TEXTURE2D(tintTex), float4 tintTexelSize, float4 tintUVScale, float4 tintFade, float tintBlend, float pixel, int vectorComponent)
      {
         float2 uv = config.uv;
         #if _MICROMESH
            uv = InverseLerp(_UVMeshRange.xy, _UVMeshRange.zw, uv);
         #endif
         uv = (uv + tintUVScale.zw) * tintUVScale.xy + noiseUV;
         half4 tex = SampleGlobalTintTex(uv, tintTex, tintTexelSize);
         COUNTSAMPLE

         float str = tintBlend * tex.a;
         float fade = saturate((camDist - tintFade.x) / max(tintFade.y, 0.01));
         str *= lerp(tintFade.z, tintFade.w, fade);

         str *= slopeFilter;

         SAMPLE_PER_TEX(perTexGeoStr, pixel, config, half4(1.0, 1.0, 1.0, 1.0));
         #if _GLOBALTINTMULT2X
            s.albedo0.rgb = lerp(s.albedo0.rgb, BlendMult2X(s.albedo0.rgb, tex.rgb), str * perTexGeoStr0[vectorComponent]);
            s.albedo1.rgb = lerp(s.albedo1.rgb, BlendMult2X(s.albedo1.rgb, tex.rgb), str * perTexGeoStr1[vectorComponent]);
            #if !_MAX2LAYER
               s.albedo2.rgb = lerp(s.albedo2.rgb, BlendMult2X(s.albedo2.rgb, tex.rgb), str * perTexGeoStr2[vectorComponent]);
            #endif
            #if !_MAX2LAYER && !_MAX3LAYER
               s.albedo3.rgb = lerp(s.albedo3.rgb, BlendMult2X(s.albedo3.rgb, tex.rgb), str * perTexGeoStr3[vectorComponent]);
            #endif
         #elif _GLOBALTINTOVERLAY
            s.albedo0.rgb = lerp(s.albedo0.rgb, BlendOverlay(s.albedo0.rgb, tex.rgb), str * perTexGeoStr0[vectorComponent]);
            s.albedo1.rgb = lerp(s.albedo1.rgb, BlendOverlay(s.albedo1.rgb, tex.rgb), str * perTexGeoStr1[vectorComponent]);
            #if !_MAX2LAYER
               s.albedo2.rgb = lerp(s.albedo2.rgb, BlendOverlay(s.albedo2.rgb, tex.rgb), str * perTexGeoStr2[vectorComponent]);
            #endif
            #if !_MAX2LAYER && !_MAX3LAYER
               s.albedo3.rgb = lerp(s.albedo3.rgb, BlendOverlay(s.albedo3.rgb, tex.rgb), str * perTexGeoStr3[vectorComponent]);
            #endif
         #elif _GLOBALTINTLIGHTCOLOR
            s.albedo0.rgb = lerp(s.albedo0.rgb, BlendLighterColor(s.albedo0.rgb, tex.rgb), str * perTexGeoStr0[vectorComponent]);
            s.albedo1.rgb = lerp(s.albedo1.rgb, BlendLighterColor(s.albedo1.rgb, tex.rgb), str * perTexGeoStr1[vectorComponent]);
            #if !_MAX2LAYER
               s.albedo2.rgb = lerp(s.albedo2.rgb, BlendLighterColor(s.albedo2.rgb, tex.rgb), str * perTexGeoStr2[vectorComponent]);
            #endif
            #if !_MAX2LAYER && !_MAX3LAYER
               s.albedo3.rgb = lerp(s.albedo3.rgb, BlendLighterColor(s.albedo3.rgb, tex.rgb), str * perTexGeoStr3[vectorComponent]);
            #endif
         #else // cross fade
            s.albedo0.rgb = lerp(s.albedo0.rgb, tex.rgb, str * perTexGeoStr0[vectorComponent]);
            s.albedo1.rgb = lerp(s.albedo1.rgb, tex.rgb, str * perTexGeoStr1[vectorComponent]);
            #if !_MAX2LAYER
               s.albedo2.rgb = lerp(s.albedo2.rgb, tex.rgb, str * perTexGeoStr2[vectorComponent]);
            #endif
            #if !_MAX2LAYER && !_MAX3LAYER
               s.albedo3.rgb = lerp(s.albedo3.rgb, tex.rgb, str * perTexGeoStr3[vectorComponent]);
            #endif
         #endif
      }

      #if _GLOBALTINT2
      #define BLEND_GLOBAL_TINT(albedo, config, camDist, slopeFilter, noiseUV) \
         GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex,  _GlobalTintTex_TexelSize, _GlobalTintUVScale, _GlobalTintFade, _GlobalTextureParams.x); \
         GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x); \

      #elif _GLOBALTINT3
         #define BLEND_GLOBAL_TINT(albedo, config, camDist, slopeFilter, noiseUV) \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex,  _GlobalTintTex_TexelSize, _GlobalTintUVScale, _GlobalTintFade, _GlobalTextureParams.x); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y); \
         ...
         #define BLEND_GLOBAL_TINT(albedo, config, camDist, slopeFilter, noiseUV) \
            ...
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex2, _GlobalTintTex2_TexelSize, _GlobalTintUVScale2, _GlobalTintFade2, _GlobalTintBlends.x); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex4, _GlobalTintTex4_TexelSize, _GlobalTintUVScale4, _GlobalTintFade4, _GlobalTintBlends.z); \
         ...
         #define BLEND_GLOBAL_TINT(albedo, config, camDist, slopeFilter, noiseUV) \
            ...
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex3, _GlobalTintTex3_TexelSize, _GlobalTintUVScale3, _GlobalTintFade3, _GlobalTintBlends.y); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex4, _GlobalTintTex4_TexelSize, _GlobalTintUVScale4, _GlobalTintFade4, _GlobalTintBlends.z); \
            GlobalTintTexture(albedo, config, camDist, slopeFilter, noiseUV, _GlobalTintTex5, _GlobalTintTex5_TexelSize, _GlobalTintUVScale5, _GlobalTintFade5, _GlobalTintBlends.w); \
      ...
      void GlobalTintTexture(inout half3 albedo, Config config, float camDist, float slopeFilter, float2 noiseUV,
         TEXTURE2D(tintTex), float4 tintTexelSize, float4 tintUVScale, float4 tintFade, float tintBlend)
      {
      ...
      void SampleSplats(float2 controlUV, inout half4 w0, inout half4 w1, inout half4 w2, inout half4 w3, inout half4 w4, inout half4 w5, inout half4 w6, inout half4 w7)
      {
            ...
            // Point sample control maps so binary splat stamps (driveways/slabs)
            // stay hard-edged. Linear sampling softens every texel boundary.

...
namespace Zombera.Debugging.DebugTools
{
    ...
    {
        [Header("Prefabs")] [SerializeField] private GameObject zombiePrefab;

        [SerializeField] private GameObject survivorPrefab;
        [SerializeField] private GameObject lootContainerPrefab;

        [Header("Systems")] [SerializeField] private ZombieManager zombieManager;
        ...
        public void SpawnZombie()
        {
            ...
            var spawnPosition = ResolveOffsetPosition(1.5f);
        ...
        public void SpawnSurvivor()
        {
            ...
            var spawnPosition = ResolveOffsetPosition(2f);
            var rotation = Quaternion.Euler(0f, Random.Range(0f, 360f), 0f);
            var parent = ResolveSpawnParent();

            var survivorObject = parent != null
                ? Instantiate(survivorPrefab, spawnPosition, rotation, parent)
                : Instantiate(survivorPrefab, spawnPosition, rotation);

            if (survivorObject != null) DebugLogger.Log(LogCategory.Squad, "SpawnSurvivor() completed", this);
        }
        ...
        public void SpawnLootContainer()
        {
            ...
            var spawnPosition = ResolveOffsetPosition(2.5f);
        ...
        public void SpawnZombieHorde(int count)
        {
            ...
            var hordeCount = Mathf.Max(1, count);
            var center = ResolveSpawnOrigin();
            var successCount = 0;

            for (var i = 0; i < hordeCount; i++)
            {
                var offset = Random.insideUnitCircle * Mathf.Max(0.1f, hordeSpawnRadius);
                var spawnPosition = center + new Vector3(offset.x, 0f, offset.y);

                if (SpawnZombieAt(spawnPosition, false)) successCount++;
            }

            DebugLogger.Log(LogCategory.World, $"SpawnZombieHorde({hordeCount}) completed: {successCount} spawned",
                this);
        }
        ...
        private bool SpawnZombieAt(Vector3 spawnPosition, bool logOutcome)
        {
            ...
            if (zombieManager != null) zombie = zombieManager.SpawnZombie(debugZombieType, spawnPosition);
            ...
            if (zombie == null && zombiePrefab != null)
            {
                ...
                var zombieObject = parent != null
                    ? Instantiate(zombiePrefab, spawnPosition, rotation, parent)
                    : Instantiate(zombiePrefab, spawnPosition, rotation);

                zombie = zombieObject != null ? zombieObject.GetComponent<ZombieController>() : null;
                zombie?.Initialize();
            }

            if (!logOutcome) return zombie != null;

            if (zombie != null)
                DebugLogger.Log(LogCategory.World, "SpawnZombie() completed", this);
            else
                DebugLogger.LogWarning(LogCategory.World,
                    "SpawnZombie() failed (no manager/spawner/prefab path succeeded)", this);
        ...
        private void AutoWireRuntimeReferences()
        {
            ...
            if (zombieSpawner == null) zombieSpawner = FindFirstObjectByType<ZombieSpawner>();

            if (lootManager == null) lootManager = FindFirstObjectByType<LootManager>();

            if (worldLootSpawner == null) worldLootSpawner = FindFirstObjectByType<LootSpawner>();
        }
        ...
        {
            if (!parentDebugSpawns) return null;

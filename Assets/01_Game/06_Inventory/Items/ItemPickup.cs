...
namespace Zombera.Inventory
{
    ...
    {
        // Push registration so save providers don't need FindObjectsByType world scans.
        private static readonly HashSet<ItemPickup> Active = new();

        /// <summary>All enabled pickups in loaded scenes (matches FindObjectsByType with inactive excluded).</summary>
        public static IReadOnlyCollection<ItemPickup> ActivePickups => Active;
        ...
        public void Initialize(ItemDefinition definition, int stackQuantity)
        {
            ...
            if (!spawnVisualFromItemDefinition) return;
        ...
        {
            if (inventory == null || itemDefinition == null) return false;

            if (!inventory.AddItem(itemDefinition, Mathf.Max(1, quantity))) return false;
        ...
        {
            if (!spawnVisualFromItemDefinition || itemDefinition == null ||
                itemDefinition.worldPickupPrefab == null) return;

            if (spawnedVisualRoot != null) return;
